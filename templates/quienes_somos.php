<?php //pass ?>
<div class="quienesSomos" id="quienes_somos">
	<div class="quienSomosCentral">
		<div class="titleQuienesSomos">
			<div class="letraIEquipo"><i class="fa-solid fa-magnifying-glass"></i></div>
			<h2>¿Quiénes Somos?</h2>
		</div>
		<p>Somos tu solución en un click; nos encargamos en brindarte el mejor servicio en materia de empresas y tecnología con un personal amplio y especializado teniendo como pilar, la calidad en todos los sentidos.</p>
	</div>
	<div class="misionVision">
		<div class="bloqueMisionVision">
			<img src="/images/misionVision/mision.jpg">
			<div class="contenido">
				<h3>Nuestra misión</h3>
				<p>Ofrecer servicios con las tecnologías más innovadoras de acuerdo con las necesidades de cada cliente, con el objetivo de máximizar su competitividad y productividad utilizando soluciones prácticas y eficientes.</p>
			</div>
		</div>
		<div class="linea"></div>
		<div class="bloqueMisionVision">
			<img src="/images/misionVision/vision.jpg">
			<div class="contenido">
				<h3>Nuestra visión</h3>
				<p>Posicionarnos como una empresa de servicios de calidad, excelencia e integralidad, como un aliado comercial estratégico aportando valor en nuestros productos y servicios de manera satisfactoria.</p>
			</div>
		</div>
	</div>
</div>